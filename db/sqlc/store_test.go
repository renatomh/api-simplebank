...
func TestTransferTxDeadlock(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		// We'll change the accounts transfer direction every two items
		if i%2 == 1 {
			fromAccountID = account2.ID
			toAccountID = account1.ID
		}

		// Executing the transaction asynchronoulsy
		go func() {
			// We've lready checked returned results in the previous test
			// Here, we only care about testing for deadlocks
